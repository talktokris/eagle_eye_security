						...
						<div class="card">
							<div class="card-body">
								<div class="p-4 border rounded">
                                    ...
                                    <form class="needs-validation" method="post" enctype="multipart/form-data" action="{{ url('admin/client/create-client') }}" >
                                            @csrf

                                            <div class="col-md-12 mb-3">
                                                <label for="validationCustom03" class="form-label">Client Name :</label>
                                                <input type="text" name="client_name" class="form-control @error('client_name') is-invalid @enderror" id="validationCustom03" value="{{ old('client_name') }}" required="">
                                            ...
                                            <div class="col-md-12 mb-3">
                                                <label for="client_category" class="form-label">Please Select Client Category : </label>
                                                <select name="client_category" class="form-select" id="validationCustom04" required="">
                                                    <?php if(old('client_category')){
                                                        if(old('client_category')==0){ $status_string='Not Active';}
                                                        elseif(old('client_category')==1){ $status_string='Active';}
                                                        else { $status_string='';}
                                                        ?>
                                                         <option value="<?php echo old('client_category') ?>"><?php echo old('client_category') ?></option>
                                                        <?php  }  ?>
